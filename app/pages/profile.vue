<template>
  <div
    class="product-viewer-bg"
    :style="{
      backgroundImage: groups[currentGroup]?.detail?.colors
        ? `url(${groups[currentGroup]?.detail?.colors?.[currentColorIdx]?.img || groups[currentGroup]?.image})`
        : `url(${groups[currentGroup]?.image})`,
...
function selectGroup(idx: number) {
  ...
  navTimeout = window.setTimeout(() => { showNav.value = true }, 350)
}
...
<style scoped>
  .product-viewer-bg .nav-btn-mobile {
    position: absolute;
    top: 50%;
  ...
  .product-viewer-bg .nav-btn-mobile.left {
    left: 0.8rem;
  }
  .product-viewer-bg .nav-btn-mobile.right {
    right: 0.8rem;
  }
...
.sidebar-group {
  ...
  .product-viewer-bg .mobile-bottom-bar {
    position: absolute;
    left: 0;
    bottom: 0;
    width: 100vw;
...
.sidebar-groups-mobile {
  ...
  .plus-icon {
    ...
    height: 2.1rem;
    border-radius: 50%;
    background: linear-gradient(135deg, #eaf2fb 0%, #f5f5f7 100%);
    box-shadow: 0 2px 8px rgba(0,113,227,0.08);
    transition: background 0.3s, box-shadow 0.3s;
    flex-shrink: 0;
  }
  .sidebar-btn:hover .plus-icon {
    background: linear-gradient(135deg, #e3eafc 0%, #eaf2fb 100%);
  ...
  .plus-shape {
    ...
    justify-content: center;
    user-select: none;
    text-shadow: 0 1px 4px rgba(0,113,227,0.10);
  }
...
.product-viewer-main {
  ...
  .product-bg-image {
    ...
    border-radius: 2vw;
    z-index: 1;
    transition: background-image 0.6s cubic-bezier(0.22, 1, 0.36, 1);
  }
...
.product-viewer-bg .sidebar-fade-enter-from {
  opacity: 0;
  filter: blur(24px);
  transform: scale(0.96) translateY(4vh) skewY(2deg);
}
.product-viewer-bg .sidebar-fade-enter-to {
  opacity: 1;
  transform: translateY(0) scale(1) skewY(0deg);
  filter: blur(0);
}
.product-viewer-bg .sidebar-fade-leave-from {
  opacity: 1;
  transform: translateY(0) scale(1) skewY(0deg);
  filter: blur(0);
}
.product-viewer-bg .sidebar-fade-leave-to {
  opacity: 0;
  filter: blur(24px);
  transform: scale(1.04) translateY(-4vh) skewY(-2deg);
}
.product-viewer-bg .btn-slide-enter-active,
.product-viewer-bg .btn-slide-leave-active {
  transition: all 0.4s cubic-bezier(.4,0,.2,1);
}
.product-viewer-bg .btn-slide-enter-from,
.product-viewer-bg .btn-slide-leave-to {
  opacity: 0;
  transform: translateX(-32px) scale(0.92);
}
.product-viewer-bg .btn-slide-enter-to,
.product-viewer-bg .btn-slide-leave-from {
  opacity: 1;
  transform: translateX(0) scale(1.04);
}
.product-viewer-bg .btn-slide-x-enter-active,
.product-viewer-bg .btn-slide-x-leave-active {
  transition: all 0.4s cubic-bezier(.4,0,.2,1);
}
.product-viewer-bg .btn-slide-x-enter-from,
.product-viewer-bg .btn-slide-x-leave-to {
  opacity: 0;
  transform: translateY(32px) scale(0.92);
}
.product-viewer-bg .btn-slide-x-enter-to,
.product-viewer-bg .btn-slide-x-leave-from {
  opacity: 1;
  transform: translateY(0) scale(1.04);
}
.product-viewer-bg .card-fade-enter-active,
.product-viewer-bg .card-fade-leave-active {
  transition: opacity 0.5s cubic-bezier(.4,0,.2,1), transform 0.5s cubic-bezier(.4,0,.2,1);
}
.product-viewer-bg .card-fade-enter-from,
.product-viewer-bg .card-fade-leave-to {
  opacity: 0;
  transform: translateY(24px);
}
.product-viewer-bg .card-fade-enter-to,
.product-viewer-bg .card-fade-leave-from {
  opacity: 1;

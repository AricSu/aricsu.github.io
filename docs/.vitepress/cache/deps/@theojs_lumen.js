...
function parseLoaderResponse(storage3, icons, data, isAPIResponse) {
    ...
    setTimeout(() => {
      ...
      if (storage3.loadIcons && (icons2.length > 1 || !customIconLoader)) {
        parsePossiblyAsyncResponse(
          ...
          (data) => {
            parseLoaderResponse(storage3, icons2, data, false);
          }
        );
        return;
      }
      if (customIconLoader) {
        icons2.forEach((name) => {
          ...
          parsePossiblyAsyncResponse(response, (data) => {
            ...
            parseLoaderResponse(storage3, [name], iconSet, false);
          });
        });
        return;
      }
      ...
      if (invalid.length) {
        parseLoaderResponse(storage3, invalid, null, false);
      }
      if (!valid.length) {
        return;
      }
      ...
      if (!api) {
        parseLoaderResponse(storage3, valid, null, false);
        return;
      }
      const params = api.prepare(provider, prefix, valid);
      params.forEach((item) => {
        sendAPIQuery(provider, item, (data) => {
          parseLoaderResponse(storage3, item.icons, data, true);
        });
      });
    });
  }
...
function parseLoaderResponse2(storage3, icons, data, isAPIResponse) {
  function checkMissing() {
    const pending = storage3.pendingIcons;
    icons.forEach((name) => {
      if (pending) {
    ...
    setTimeout(() => {
      ...
      if (storage3.loadIcons && (icons2.length > 1 || !customIconLoader)) {
        parsePossiblyAsyncResponse2(
          storage3.loadIcons(icons2, prefix, provider),
          (data) => {
            parseLoaderResponse2(storage3, icons2, data, false);
          }
        );
        return;
      }
      if (customIconLoader) {
        icons2.forEach((name) => {
          ...
          parsePossiblyAsyncResponse2(response, (data) => {
            ...
            parseLoaderResponse2(storage3, [name], iconSet, false);
          });
        });
        return;
      }
      ...
      if (invalid.length) {
        parseLoaderResponse2(storage3, invalid, null, false);
      }
      if (!valid.length) {
        return;
      }
      ...
      if (!api) {
        parseLoaderResponse2(storage3, valid, null, false);
        return;
      }
      const params = api.prepare(provider, prefix, valid);
      params.forEach((item) => {
        sendAPIQuery2(provider, item, (data) => {
          parseLoaderResponse2(storage3, item.icons, data, true);
        });
      });
    });
  }
...
function parseIconValue(value, onload) {
  ...
  const name = stringToIcon2(value, true, true);
  if (!name) {
    return {
      value
    };
...
function setCustomIconsLoader$1(loader, prefix, provider) {
  getStorage2(provider || "", prefix).loadIcons = loader;
}
function setCustomIconLoader$1(loader, prefix, provider) {
  getStorage2(provider || "", prefix).loadIcon = loader;
}

  ...
  <div :style="mainBgStyle" :class="[
    ...
    <template v-if="isMobile">
      ...
      <button v-if="currentGroup > 0"
        ...
        <svg width="28" height="28" viewBox="0 0 28 28" fill="none">
          ...
          <path d="M17 9L12 14L17 19" stroke="#888" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" />
        </svg>
      </button>
      <button v-if="currentGroup < groups.length - 1"
        ...
        <svg width="28" height="28" viewBox="0 0 28 28" fill="none">
          <circle cx="14" cy="14" r="13" fill="#f6f7fa" />
          <path d="M11 9L16 14L11 19" stroke="#222" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" />
        </svg>
      </button>
      <!-- 底部悬浮卡片 -->
      <div
        ...
        <div class="w-full flex flex-col items-center justify-center">
          ...
          <img v-if="groups[currentGroup]?.detail?.img" :src="groups[currentGroup]?.detail?.img"
            class="w-full rounded-[1vw] mt-2 object-cover" alt="detail">
        </div>
      </div>
    </template>
    <!-- 桌面端逻辑 -->
    <template v-else>
      <div class="flex flex-row items-center justify-center gap-[1vw] min-w-[22vw] max-w-[28vw] h-full pl-[2vw]">
        <div class="flex flex-col items-center justify-center gap-[2vw] h-full min-w-[3vw] max-w-[3vw]"
          ...
          <button v-if="showNav"
            ...
            :disabled="currentGroup === 0" @click="selectGroup(currentGroup - 1)">
            <svg width="28" height="28" viewBox="0 0 28 28" fill="none" xmlns="http://www.w3.org/2000/svg">
              <circle cx="14" cy="14" r="13" fill="#f6f7fa" />
              <path d="M9 16L14 11L19 16" stroke="#888" stroke-width="2.5" stroke-linecap="round"
                stroke-linejoin="round" />
            </svg>
          </button>
          <button v-if="showNav"
            class="w-[3vw] h-[3vw] min-w-[40px] min-h-[40px] max-w-[56px] max-h-[56px] rounded-full border-none bg-[#f6f7fa] shadow-[0_2px_16px_rgba(0,0,0,0.04)] flex items-center justify-center transition-all outline-none m-[0.6vw_0] p-0 disabled:opacity-45 disabled:cursor-not-allowed"
            :disabled="currentGroup === groups.length - 1" @click="selectGroup(currentGroup + 1)">
            <svg width="28" height="28" viewBox="0 0 28 28" fill="none" xmlns="http://www.w3.org/2000/svg">
              <circle cx="14" cy="14" r="13" fill="#f6f7fa" />
              <path d="M9 12L14 17L19 12" stroke="#222" stroke-width="2.5" stroke-linecap="round"
                stroke-linejoin="round" />
            </svg>
          </button>
        </div>
        <div class="flex flex-col mt-[10vh] gap-[1vw] items-start justify-center w-full">
          <transition-group name="sidebar-fade" tag="div">
            <div v-for="(group, idx) in groups" :key="group.label"
              class="flex flex-col gap-[1vw] items-start w-full m-auto">
              <transition name="card-fade" mode="out-in">
                <button v-if="idx !== currentGroup" :key="'btn-' + idx"
                  ...
                  :class="{ bgGradient: idx === currentGroup }" @click="selectGroup(idx); showNav = true">
                  <span
                ...
                <div v-else-if="group.detail && group.detail.desc" :key="'desc-' + idx"
                  ...
                  <div v-if="group.detail.colors" class="flex flex-col gap-2">
                    ...
                    <div class="flex gap-[1.2vw] mt-[1vw] justify-start">
                      <button v-for="(color, cidx) in group.detail.colors" :key="color.name"
                        ...
                        :style="{ background: color.hex }" @click.stop="selectColor(cidx)" />
                    </div>
                  </div>
                  <div v-else>
                    <div v-if="group.detail.title" class="font-bold text-[1.1rem] mb-1">
                      {{ group.detail.title }}
                    </div>
                    <div class="text-[#222] leading-[1.6]">
                      {{ group.detail.desc }}
                    </div>
                    <img v-if="group.detail.img" :src="group.detail.img"
                      class="w-full rounded-[1vw] mt-[0.8vw] object-cover" alt="detail">
                  </div>
                </div>
              </transition>
            </div>
...
const groups: Group[] = [
  {
    ...
    imageMobile: '/images/ocp_mobile.jpg',
    detail: {
      title: 'OCP Certified',
  ...
  },
  {
    ...
    image: '/images/pctp_chinese.jpg',
    imageMobile: '/images/pctp_chinese_mobile.jpg',
    detail: {
      title: 'PCTP Certified (中文)',
      desc: '通过中文版本的 PCTP 认证，具备分布式平台技术的系统性能力，适用于本地化场景和中国市场需求。',
      img: '/images/pctp_chinese_mobile.jpg'
    }
  },
  {
    label: 'PTCP Certified - English Version',
    image: '/images/ptcp_english.jpg',
    imageMobile: '/images/ptcp_english_mobile.jpg',
    detail: {
  ...
  },
  {
    ...
    image: '/images/ptca.jpg',
    imageMobile: '/images/ptca_mobile.jpg',
    detail: {
  ...
  },
  {
    ...
    image: '/images/pgce.jpg',
    imageMobile: '/images/pgce_mobile.jpg',
    detail: {
  ...
  },
  {
    ...
    imageMobile: '/images/pgca_mobile.jpg',
    detail: {
  ...
  },
  {
    ...
    image: '/images/mva.jpg',
    imageMobile: '/images/mva_mobile.jpg',
    detail: {

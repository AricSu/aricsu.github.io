  ...
  CapturingGroup: {
    enter({
      ...
      if (origin && openRefs.has(ref)) {
        const recursion2 = createRecursion(ref);
        reffedNodesByReferencer.set(recursion2, openRefs.get(ref));
...
// node_modules/.pnpm/regex-recursion@4.3.0/node_modules/regex-recursion/src/index.js
var r3 = String.raw;
...
function repeatWithDepth(expression, reps, namesInRecursed, direction = "forward") {
  ...
  for (let i = 0; i < reps; i++) {
    ...
    result += replaceUnescaped(
      expression,
      r3`${namedCapturingDelim}|\\k<(?<backref>[^>]+)>`,
      ({ 0: m, groups: { captureName, backref } }) => {
        if (backref && namesInRecursed && !namesInRecursed.has(backref)) {
          return m;
        }
        const suffix = `_$${captureNum}`;
        return captureName ? `(?<${captureName}${suffix}>` : r3`\k<${backref}${suffix}>`;
      },
      Context.DEFAULT
    );
  }
